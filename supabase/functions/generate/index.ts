...
function buildPrompt(
  ...
  const system =
    ...
    "- Если в правилах группы задана конкретная фраза (например, с чего начинать поздравление) — воспроизводи её ДОСЛОВНО, без перефразирования.\n" +
  ...
  // Плейсхолдер ИМЯ (заглавными) заменяем на обращение детерминированно,
  // до промпта: пользователь может задать в правиле группы конкретную фразу.
  const displayName = (contact.call_name as string | null)?.trim() || (contact.name as string);
  const substitutedVectors = wishVectors.map((v) => v.replace(/ИМЯ/g, displayName));
  const wishBlock = substitutedVectors.length
    ? `- Общие пожелания/правила групп этого контакта (учитывай наравне с фактами; конкретные фразы — воспроизводи дословно): ${substitutedVectors.join("; ")}\n`
    : "";
